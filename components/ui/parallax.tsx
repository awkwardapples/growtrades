...
export const Parallax = ({ className, ...props }: React.ComponentProps<'div'>) => {
  return <div className={cn('relative min-h-dvh w-full', className)} {...props} />;
};

export function ParallaxContainer({ className, ...props }: React.ComponentProps<'div'>) {
  return <div className={cn('min-h-screen', className)} {...props} />;
}

export function ParallaxItem({ start, end, className, style, ...props }: ParallaxItemProps) {
